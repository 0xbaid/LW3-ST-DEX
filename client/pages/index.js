...
export default function Home() {
  ...
  // amount of Eth held by the user's account
  const [ethBalance, setEthBalance] = useState(zero);
  // Crypto Dev tokens Reserve balance in the Exchange contract
  const [reservedCD, setReservedCD] = useState(zero);
  // ether balance in the contract
  const [etherBalanceContract, setEtherBalanceContract] = useState(zero);
  // amount of `CD` tokens help by the users account
  const [cdBalance, setCDBalance] = useState(zero);
  // amount of LP tokens held by the users account
  const [lpBalance, setLPBalance] = useState(zero);
  // amount of Ether that the user wants to add to the liquidity
  const [addEther, setAddEther] = useState(zero);
  // amount of CD tokens that the user wants to add to the liquidity
  const [addCDTokens, setAddCDTokens] = useState(zero);
  // amount of `Ether` that would be sent back to the user based on a certain number of `LP` tokens
  const [removeEther, setRemoveEther] = useState(zero);
  // tokens that would be sent back to the user based on a certain number of `LP` tokens
  ...
  // tokens that the user would receive after a swap completes
